...
namespace MelonLoader.Installer
{
    static class Program
    {
        ...
        private static void Install_Legacy_02(string dirpath, string selectedVersion)
        {
            ...
            string tempfilepath = TempFileCache.CreateFile();
            webClient.DownloadFileAsync(new Uri("https://github.com/HerpDerpinstine/MelonLoader/releases/download/" + selectedVersion + "/MelonLoader" + (is_02 ? "_" : ".") + (File.Exists(Path.Combine(dirpath, "UserAssembly.dll")) ? "Il2Cpp" : "Mono") + ".zip"), tempfilepath);
            while (webClient.IsBusy) { }
            SetDisplayText("Extracting MelonLoader...");
            Cleanup(dirpath);
            ExtractZip(dirpath, tempfilepath);
